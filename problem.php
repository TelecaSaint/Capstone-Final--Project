...
.prob-q{font-family:'Syne',sans-serif;font-weight:800;font-size:clamp(1.2em,3vw,2em);color:var(--text);margin-bottom:6px;line-height:1.35}
...
@media(max-width:600px){.sidebar{grid-template-columns:1fr}nav{padding:0 14px}.prob-q{font-size:1.2em}}
</style>
</head>
<body>
<nav>
...
function beep(f, d=0.15, vol=0.08, type='sine'){
  try{
...
let hintUsed = false, aiUsed = false;
let timerSecs = 150, timerInt = null, startTime = Date.now();

// ── THE FIX: answered flag prevents double-submit and stale submits ──
let answered = false;
// ── loading flag prevents nextProblem() firing during fetch ──
let loading  = false;

const results = [];

// ── Monster + quest config ────────────────────────────────────
const monsters = {
...
function getFallback(subject, difficulty){
  ...
  return bank[difficulty] || bank.medium || Object.values(bank)[0];
}

// ── URL param — set subject from link ─────────────────────────
...
const diffDescs = {
  easy:   'Simple 1-step problems',
  medium: 'Medium problems with 2-step equations',
  hard:   'Multi-step challenge problems',
};
...
// ── Core reset — called at the start of every new question ────
function resetQuestionUI(){
  ...
  submitBtn.disabled = true;          // enabled once problem loads
  startTime = Date.now();
}

// ── Fetch AI problem ──────────────────────────────────────────
async function fetchAIProblem(){
  if(loading) return;                 // guard against re-entrant calls
  loading = true;
  ...
  const subjectLabel = currentSubject === 'wordproblems' ? 'word problem' : currentSubject;
  const prompt = `Generate a ${currentDiff} ${subjectLabel} math problem for a middle school student.
Return ONLY valid JSON:
{"type":"short label","question":"the problem","answer":"exact answer only","hint":"one-sentence hint","explanation":"step-by-step 3-4 lines using \\n"}
No markdown, no extra text.`;
  ...
  try {
    ...
    currentProblem = JSON.parse(raw);
  } catch(e){
    // Fallback: pick a random problem from the local bank
    const pool = getFallback(currentSubject, currentDiff);
...
async function submitAnswer(){
  // Guard: don't submit if loading, already answered, or no problem loaded
  if(loading || answered || !currentProblem) return;
  ...
  const isCorrect = userAns.toLowerCase() === correct
                    || parseFloat(userAns) === parseFloat(correct);
  const xpEarned  = isCorrect ? (hintUsed ? 25 : 50) : 0;

  // Save to DB via api.php
  ...
  fetch('api.php', {method:'POST', body:fd})
    ...
    .then(d => {
      ...
      else if(d.xp !== undefined){ userXP = d.xp;    streak = d.streak; }
...
function showResult(win, xp){
  ...
  document.getElementById('resSub').textContent = win
    ? 'Great work! Moving to next problem.'
...
function nextProblem(){
  if(loading) return;                 // wait for any in-flight fetch to finish
  ...
  if(probIndex >= 10){
    alert('🎉 Quest Complete! +200 XP Bonus!');
...
function updateTimerDisplay(){
  ...
  el.className   = 'timer-display' + (timerSecs <= 10 ? ' crit' : timerSecs <= 30 ? ' warn' : '');
}

// ── Progress dots ─────────────────────────────────────────────
function updateDots(){
  const c = document.getElementById('probDots');
  c.innerHTML = '';
  for(let i = 0; i < 10; i++){
